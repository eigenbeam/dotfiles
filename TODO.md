...
- [x] **direnv** — Per-project environment auto-loading. `cd` into a project and `.envrc` sets up env vars, PATH, virtualenvs automatically. Pairs well with `uv` (`layout python`). https://github.com/direnv/direnv

- [x] **zoxide** — Smarter `cd` that learns directory habits. `z dot` jumps to `~/projects/dotfiles` from anywhere. https://github.com/ajeetdsouza/zoxide
...
- [x] **Neovim with lazy.nvim** — Kickstart-inspired config with Telescope, LSP, Treesitter, blink.cmp, gitsigns. Base2Tone EveningDark theme.
